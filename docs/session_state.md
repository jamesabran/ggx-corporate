...
**NEXT (session ended here per user request — resume with):** Support Tickets (`420:70`) → Service Advisories (`420:71`). Code = `SupportTickets.tsx` / `SupportTicketDetail.tsx`, `ServiceAdvisories.tsx`. Reuse the Badge/Button helpers + primary-button fix documented above, and **heed the setCurrentPageAsync gotcha**.
