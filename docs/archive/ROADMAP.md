...
| **3 (in progress)** | Migrate UI consumers to service layer | **Well advanced (2026-05-31).** 11 services exist; most domain pages migrated (transactions, dashboard, subaccounts, users, bulk upload, notifications, claims, SLA, reports, earnings, support tickets, **data analytics, parent dashboard**). **Remaining:** BulkUploadSummary, RootLayout search (services exist for these), then AuthContext last. No-service-yet: service advisories, payment accounts, financial security. See `MOCK_SERVICE_LAYER.md` §5–§7. |
...
- ✅ ParentDashboard migrated (2026-05-31) → `slaService.getSlaAlertsList({ openOnly: true })`. `data/slaAlerts` now has no direct page importers.
- **Next: BulkUploadSummary** (`getSessionUploads` → `bulkUploadService`).
